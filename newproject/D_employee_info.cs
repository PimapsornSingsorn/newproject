    ...
    public partial class employee_info : Form
    {
        ...
        private void Employee_info()
        {
            try
            {
                ...
                cmd.CommandText = "select * from ข้อมูลพนักงาน	";
        ...
        private void submit_btn_Click(object sender, EventArgs e)
        {
            try
            {
                if (fname_txt.Text == "" || lname_txt.Text == "" || phone_num_txt.Text == "" || address_txt.Text == "")
                {
                    MessageBox.Show("กรุณากรอกข้อมูลให้ครบ !", "ตรวจพบข้อผิดพลาด", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else if (phone_num_txt.Text.Length != 10)
                {
                ...
                else
                {
                    ...
                    string sql = "INSERT INTO ข้อมูลพนักงาน (ชื่อ,สกุล,เบอร์โทร,ที่อยู่,วันนำเข้า) VALUES('" + fname_txt.Text + "','" + lname_txt.Text + "','" + phone_num_txt.Text + "','" + address_txt.Text + "','" + this.dateTimePicker1.Value.ToString("dd-MM-yyyy") + "')";
        ...
        private void delete_btn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dialogResult == DialogResult.OK)
                {
                    ...
                    int deleteId = Convert.ToInt32(dataGridView1.Rows[selectedRow].Cells["รหัสพนักงาน"].Value);
                    MySqlConnection conn = databaseConnection();
                    string sql = "DELETE FROM ข้อมูลพนักงาน WHERE รหัสพนักงาน = '" + deleteId + "'";
                    MySqlCommand cmd = new MySqlCommand(sql, conn);
                    conn.Open();
                    int rows = cmd.ExecuteNonQuery();
                    conn.Close();
        ...
        private void edit_btn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    int selectedRow = dataGridView1.CurrentCell.RowIndex;
                    int editId = Convert.ToInt32(dataGridView1.Rows[selectedRow].Cells["รหัสพนักงาน"].Value);

                    MySqlConnection conn = databaseConnection();

                    string sql = "UPDATE ข้อมูลพนักงาน SET ชื่อ = '" + fname_txt.Text + "',สกุล = '" + lname_txt.Text + "',เบอร์โทร = '" + phone_num_txt.Text + "',ที่อยู่ = '" + address_txt.Text + "' WHERE รหัสพนักงาน = '" + editId + "'";
                    MySqlCommand cmd = new MySqlCommand(sql, conn);
                    conn.Open();
        ...
        private void phone_num_txt_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void search_btn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MySqlCommand cmd;
                cmd = connection.CreateCommand();
                cmd.CommandText = "select รหัสพนักงาน,ชื่อ,สกุล,เบอร์โทร,ที่อยู่,วันนำเข้า from ข้อมูลพนักงาน where ชื่อ like  '%" + search_txt.Text + "%'";
                MySqlDataAdapter adapter = new MySqlDataAdapter(cmd);
                DataSet ds = new DataSet();
                adapter.Fill(ds);
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            phone_num_txt.Text = dataGridView1.Rows[e.RowIndex].Cells["เบอร์โทร"].FormattedValue.ToString();
